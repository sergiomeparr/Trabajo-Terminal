...
@WebServlet(name = "NuevoRanking", urlPatterns = {"/NuevoRanking"})
public class NuevoRanking extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            r.setIdPaciente(usuario);
            r.setIdHospital(hospital);
            
            //Crecion del rankeo
            RankeoCRUD dao = new RankeoCRUD();
            try {
                dao.crear(r);
                response.sendRedirect("/Prototipo4_1/Paginas/Informacion_Hospital.jsp?id=" + hospital + "&usuario="+usuario);
